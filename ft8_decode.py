...
# The permutation is generated by bit-reversing the index in an 8-bit field.
# This table will be computed at module load by _build_ft8_interleave_table().
...
def _invert_perm_dst_to_src(perm_dst_to_src: tuple[int, ...]) -> tuple[int, ...]:
    """
    Invert a permutation stored as perm[dst] = src into inv[src] = dst.
    """
    inv = [0] * len(perm_dst_to_src)
    for dst, src in enumerate(perm_dst_to_src):
        inv[int(src)] = int(dst)
    return tuple(inv)

# TX interleaver mapping from ft8_lib: interleaved[dst] = codeword[src]
_FT8_INTERLEAVE_DST_TO_SRC: tuple[int, ...] = _build_ft8_interleave_table()

# RX needs the inverse mapping: codeword[src] = rx_interleaved[inv[src]]
_FT8_DEINTERLEAVE_SRC_TO_DST: tuple[int, ...] = _invert_perm_dst_to_src(_FT8_INTERLEAVE_DST_TO_SRC)
...
def ft8_gray_decode(
    ...
    Sign convention: positive LLR = bit 1 likely, negative LLR = bit 0 likely.
    ...
    for s in range(N_syms):
        ...
        # Per-symbol noise floor for LLR scaling.
        # Mean energy across all 8 tones gives the noise reference; dividing
        # makes LLRs dimensionless and properly scaled for LDPC convergence.
        noise_floor = max(float(np.mean(row)), 1e-30)

        for b in range(3):
            hard_bits_raw[3 * s + b] = int((gv >> (2 - b)) & 1)

            # Correct 8-FSK soft LLR: log(sum E[tones with bit=1])
            #                       - log(sum E[tones with bit=0])
            # Normalised by noise floor so LDPC receives well-scaled soft values.
            sum_e1 = max(sum(float(row[k]) for k in bits1[b]), 1e-30)
            sum_e0 = max(sum(float(row[k]) for k in bits0[b]), 1e-30)
            llrs_raw[3 * s + b] = math.log(sum_e1 / sum_e0)

    # Apply 174-bit DE-interleaver.
    # TX: interleaved[dst] = codeword[src]  where _FT8_INTERLEAVE_DST_TO_SRC[dst]=src
    # RX: codeword[src] = interleaved[_FT8_DEINTERLEAVE_SRC_TO_DST[src]]
    inv = _FT8_DEINTERLEAVE_SRC_TO_DST  # inv[src] = dst
    hard_bits = hard_bits_raw[list(inv)]
    llrs = llrs_raw[list(inv)]
...
def ft8_ldpc_decode(
    ...
    *,
    ...
    llrs          : (174,) float64 — channel LLRs from ft8_gray_decode.
                    Sign convention: positive = likely bit 1, negative = likely bit 0.
    max_iterations: number of belief-propagation iterations (default 100).
    llr_clamp     : clip LLRs to ±this value to prevent numerical overflow.
    min_sum_scale : scaling factor for min-sum approximation (default 0.75).
    ...
    bits    = (Lch > 0.0).astype(np.uint8)   # initial hard decision: bit 1 if LLR > 0
    all_ok  = False
    ...
    for iteration in range(max_iterations):
        ...
        bits = (L_post > 0.0).astype(np.uint8)   # 1 if LLR > 0 (bit 1 likely)
    ...
    # FT8 CRC-14 is computed over the 77 message bits directly (no padding).
        ...
        while not self._stop.is_set():
            ...
            for slot_start_utc, frame, is_partial in frames:
                ...
                for (t0_s, f0_hz, sc_db) in sync_cands:
                    ...
                    # Try both LLR polarities (some demapper conventions are inverted).
                    ldpc_ok, codeword, ldpc_iters = ft8_ldpc_decode(llrs)
                    llr_polarity = "+"

                    if not ldpc_ok:
                        ldpc_ok2, codeword2, ldpc_iters2 = ft8_ldpc_decode(-llrs)
                        if ldpc_ok2:
                           ldpc_ok, codeword, ldpc_iters = ldpc_ok2, codeword2, ldpc_iters2
                           llr_polarity = "-"
